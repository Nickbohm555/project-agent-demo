...
export function resolveProxyApiHost(env: NodeJS.ProcessEnv = process.env): string {
  const host = env.PAD_API_HOST?.trim();
  return host && host.length > 0 ? host : DEFAULT_API_HOST;
}
